 ...
 * OptinMmsBanner
 * ─────────────────────────────────────────────────────────────────────────────
 * Muestra el cargo de MMS de opt-in (registro de participantes en sorteos)
 * para el periodo actual del mes. Estos MMS NO aparecen en las campañas
 * normales — son cargos "invisibles" que sí se incluyen en la factura mensual.
 *
 * Precio: $0.085 por MMS enviado (cuando smsStatus !== 'skipped')
 * Los "skipped" son registros donde el cliente tenía active:false → no se cobran.
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
export default function OptinMmsBanner({ storeId }: OptinMmsBannerProps) {
  ...
  // Periodo: desde el 1 del mes actual hasta hoy
  ...
  // Color según el costo: verde < $200, amarillo < $800, rojo >= $800
  const accentColor =
  ...
  return (
    ...
    >
        ...
        <Stack direction="row" alignItems="center" spacing={1.5} flex={1} minWidth={0}>
          ...
          <Box minWidth={0} overflow="hidden">
            <Stack direction="row" alignItems="center" spacing={1}>
              ...
              <Tooltip
                title={`Cada cliente que escanea el QR o se registra recibe un MMS de confirmación ($${OPTIN_PRICE}/msg). Estos costos se incluyen en la factura mensual de la tienda como línea "optin".`}
                arrow
                placement="top"
              >
            ...
            <Typography sx={{ fontSize: 11, color: 'text.disabled' }}>
              Periodo: {label} · $0.085/msg · no aparecen como campañas
            </Typography>
          </Box>
        </Stack>
          ...
          {isLoading ? (
            ...
            <>
              ...
              {skipped > 0 && (
                <Chip
                  size="small"
                  icon={<BlockRoundedIcon sx={{ fontSize: 12 }} />}
                  label={`${skipped.toLocaleString()} omitidos`}
                  sx={{
      ...
      {/* ── Progress bar (siempre visible cuando hay datos) ── */}
      {!isLoading && total > 0 && (
        <LinearProgress
          variant="determinate"
          value={sentRate}
        ...
        >
          ...
          >
            <Stack direction="row" alignItems="center" spacing={0.75} mb={0.5}>
              ...
              <Typography sx={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'text.secondary' }}>
                MMS Enviados
              </Typography>
              <Box
                sx={{
                  ml: 'auto', px: 0.75, py: 0.2, borderRadius: 1,
                  bgcolor: alpha('#10b981', 0.1), border: `1px solid ${alpha('#10b981', 0.2)}`,
                }}
              >
                <Typography sx={{ fontSize: 10, fontWeight: 700, color: '#10b981', fontVariantNumeric: 'tabular-nums' }}>
                  {sentRate}%
                </Typography>
              </Box>
            </Stack>
            {isLoading ? (
              ...
              <Typography sx={{ fontSize: 24, fontWeight: 900, color: '#10b981', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>
                {sent.toLocaleString()}
              </Typography>
            )}
            <Typography sx={{ fontSize: 10, color: 'text.disabled', mt: 0.25 }}>
              clientes activos que recibieron el MMS
            </Typography>
          </Box>
          ...
          >
            <Stack direction="row" alignItems="center" spacing={0.75} mb={0.5}>
              ...
              {skipRate > 0 && (
                <Box
                  sx={{
                    ml: 'auto', px: 0.75, py: 0.2, borderRadius: 1,
                    bgcolor: alpha('#6b7280', 0.1), border: `1px solid ${alpha('#6b7280', 0.2)}`,
                  }}
                >
                  <Typography sx={{ fontSize: 10, fontWeight: 700, color: '#6b7280', fontVariantNumeric: 'tabular-nums' }}>
                    {skipRate}%
                  </Typography>
                </Box>
              )}
            ...
            {isLoading ? (
              ...
              <Typography sx={{ fontSize: 24, fontWeight: 900, color: 'text.secondary', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>
                {skipped.toLocaleString()}
              </Typography>
            )}
            <Typography sx={{ fontSize: 10, color: 'text.disabled', mt: 0.25 }}>
              opt-outs / inactivos · no se cobran
            </Typography>
          </Box>
          ...
          >
            ...
            <Typography sx={{ fontSize: 10, color: 'text.disabled', mt: 0.25 }}>
              participantes registrados en el periodo
            </Typography>
          </Box>
          ...
          <Box sx={{ px: 2.5, py: 1.5 }}>
            ...
            <Typography sx={{ fontSize: 10, color: 'text.disabled', mt: 0.25 }}>
              $0.085 × {sent.toLocaleString()} msg · incluido en tu factura
            </Typography>
          </Box>
        </Box>
      </Collapse>
