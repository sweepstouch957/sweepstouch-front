...
export default function CampaignArtDropzone({
  ...
  useEffect(() => {
    const onPaste = (e: ClipboardEvent) => {
      ...
      const item = Array.from(e.clipboardData?.items || []).find((i) => /^image\/(png|jpe?g)$/.test(i.type));
  ...
  return (
    ...
    >
      ...
      {!preview ? (
        // ── Vacío
        <Stack alignItems="center" justifyContent="center" gap={1.25} sx={{ py: { xs: 4, sm: 5 }, px: 2, textAlign: 'center' }}>
          <Box
            sx={{
              width: 52, height: 52, borderRadius: 2.5, display: 'grid', placeItems: 'center',
              color: 'primary.main', bgcolor: (t) => alpha(t.palette.primary.main, 0.08),
            }}
          >
          ...
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              o <Box component="span" sx={{ color: 'primary.main', fontWeight: 700 }}>haz clic para elegirlo</Box> · también puedes pegarlo con Ctrl+V
            </Typography>
          </Box>
          <Stack direction="row" gap={0.75} flexWrap="wrap" justifyContent="center">
            <Chip size="small" variant="outlined" label="JPG o PNG" />
            <Chip size="small" variant="outlined" label="Hasta 100 MB" />
            <Chip size="small" variant="outlined" icon={<CompressRoundedIcon />} label="Se comprime sola para el MMS" />
          </Stack>
        </Stack>
      ) : (
        // ── Con imagen
        <Stack direction={{ xs: 'column', sm: 'row' }} gap={2.5} sx={{ p: 2 }}>
          <Box
            sx={{
              width: { xs: '100%', sm: 180 },
          ...
          <Stack sx={{ flex: 1, minWidth: 0 }} gap={1.25}>
            <Box>
              <Typography variant="subtitle1" fontWeight={700} noWrap title={file?.name}>
                {file ? file.name : 'Arte guardado'}
              </Typography>
              <Typography variant="body2" color="text.secondary">
                {[file ? fmtSize(file.size) : '', dims ? `${dims.w}×${dims.h}` : ''].filter(Boolean).join(' · ') || 'Imagen actual de la campaña'}
              </Typography>
            </Box>

            {/* Qué va a pasar al guardar: se entiende sin leer un párrafo. */}
            <Stack gap={0.75}>
              {file && (
                <Stack direction="row" alignItems="center" gap={1} sx={{ color: heavy ? 'info.main' : 'success.main' }}>
                  {heavy ? <CompressRoundedIcon fontSize="small" /> : <CheckCircleOutlineRoundedIcon fontSize="small" />}
                  <Typography variant="body2" color="text.primary">
                    {heavy ? 'Al guardar se crea una copia de menos de 500 KB para el MMS' : 'Ya pesa menos de 500 KB: viaja tal cual en el MMS'}
                  </Typography>
                </Stack>
              )}
            ...
            <Stack direction="row" gap={1} sx={{ mt: 'auto', pt: 0.5 }} flexWrap="wrap">
              <Button
                variant="outlined"
      ...
      {preview && isDragActive && (
        <Box
          sx={{
            position: 'absolute', inset: 0, borderRadius: 3, display: 'grid', placeItems: 'center',
